...
@Repository
public class CustomerItemManagementDAOImpl implements CustomerItemManagementDAO {
//반드시 스프링의 빈으로 제대로 등록되었는지 root-context.xml을 선택하고 Beans Graph 탭을 이용해서 확인
	...
	private static String namespace
	 ="han.gyoul.shop.mapper.CustomerItemManagementMapper";
...
//	}
	...
	public List<ItemManagementDTO> customerListAll() throws Exception {
		//return session.selectList(namespace+".customerListAll");
		//대분류 번호를 가져올 때 list 객체에 담아줌
		List<ItemManagementDTO> itemList = session.selectList(namespace+".customerListAll");
		System.out.println("ItemManagementDAOImpl.customerListAll().list:"+itemList);
		return itemList;
	}
