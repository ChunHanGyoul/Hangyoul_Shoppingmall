		...
		<section id="titlename" class="qnaBoard">
			...
			<div id="infoArea">
				<section class="search">
					<form name="search" action="./NoticeSearchList" method="post">
						<fieldset>
							<legend>검색</legend>
						...
						<c:forEach var="notice" items="${noticeList}" varStatus="status">
							<tbody>
								<tr>
									...
									<td><c:if test="${!empty notice.answer_lev}">
											<c:forEach var="j" begin="0" end="${notice.answer_lev*2}"
												step="1">&nbsp;
</c:forEach>
										</c:if> <a href="./NoticeDetail?num=<c:out value="${notice.num}" />">
											<c:out value="${notice.subject}" />
									</a></td>
									<td><c:out value="${notice.name}" /></td>
									<td><c:out value="${notice.write_date}" /></td>
									<td><c:out value="${notice.read_count}" /></td>
								</tr>
							</tbody>
						</c:forEach>
					</c:when>
			...
			<div align="center">
					...
					<tbody>
						...
						<tr>
							<td colspan="5"><c:choose>
									...
									<c:otherwise>
										<a href="./NoticeMain?page=<c:out value="${page-1}"/>">[이전]</a>&nbsp;
</c:otherwise>
									...
									<c:choose>
										<c:when test="${start eq page}"> [<c:out
												value="${start}" />] </c:when>
										<c:otherwise>
											<a href="./NoticeMain?page=<c:out value="${start}" />">[<c:out
													value="${start}" />]
											</a>&nbsp;
</c:otherwise>
									</c:choose>
									...
									<c:otherwise>
										<a href="./NoticeMain?page=<c:out value="${page+1}" />">[다음]</a>
									</c:otherwise>
								</c:choose></td>
						</tr>
					</tbody>
				...
				<div class="btnJoinAreb">
					<button type="button" value="button"
						onclick="location.href='./NoticeWrite'" class="btnOk">
						글쓰기</button>
				</div>
			</div>
		</section>
