							...
							<c:forEach var="search" items="${searchNoticelist}">
								<tr>
									...
									<td><c:if test="${!empty search.answer_lev}">
										...
										href="./NoticeDetail?num=<c:out value="${search.num}" />">
					...
					<tbody>
						...
						<tr>
							<td colspan="5"><c:choose>
									...
									<c:otherwise>
										<a href="./NoticeSearch?page=<c:out value="${page-1}"/>">[이전]</a>&nbsp;
</c:otherwise>
									...
									<c:choose>
										<c:when test="${start eq page}">[<c:out
												value="${start}" />]</c:when>
										<c:otherwise>
											<a href="./NoticeSearch?page=<c:out value="${start}" />">[<c:out
													value="${start}" />]
											</a>&nbsp;
</c:otherwise>
									</c:choose>
									...
									<c:otherwise>
										<a href="./NoticeSearch?page=<c:out value="${page+1}" />">[다음]</a>
									</c:otherwise>
								</c:choose></td>
						</tr>
					</tbody>
