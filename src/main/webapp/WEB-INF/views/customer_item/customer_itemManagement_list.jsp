...
$(document).ready( function() {
			...
			$('#searchBtn').on("click", function(event) {
						self.location = "/shop/customer_item/customer_itemManagement_list"
						...
						<c:forEach items="${list}" var="itemManagementDTO">
							<tr>
								<td>${itemManagementDTO.category_name}</td>
      							 ...
      							 <td><a href='./customer_itemManagement_viewItemDetailPage?item_no=${itemManagementDTO.item_no}'>
...
<!-- 페이지를 보여주는 부분 -->
					<div class="text-center">
						...
						<c:if test="${pageMaker.prev}">
								<li><a
									href="customer_itemManagement_list${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
							</c:if>
							...
							<c:forEach begin="${pageMaker.startPage }"
								...
								<li
									...
									<a href="customer_itemManagement_list${pageMaker.makeSearch(idx)}">${idx}</a>
								</li>
							</c:forEach>

							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li><a
									href="customer_itemManagement_list${pageMaker.makeSearch(pageMaker.endPage +1) }">&raquo;</a></li>
							</c:if>
						</ul>
					</div>
